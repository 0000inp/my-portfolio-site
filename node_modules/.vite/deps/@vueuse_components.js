...
function onClickOutside2(target, handler, options = {}) {
  ...
  if (isIOS && !_iOSWorkaround2) {
    ...
    const listenerOptions = { passive: true };
    Array.from(window2.document.body.children).forEach((el) => useEventListener2(el, "click", noop, listenerOptions));
    useEventListener2(window2.document.documentElement, "click", noop, listenerOptions);
  }
  let shouldListen = true;
  const shouldIgnore = (event) => {
    return toValue(ignore).some((target2) => {
...
function useSSRWidth2() {
  const ssrWidth = hasInjectionContext() ? injectLocal(ssrWidthSymbol2, null) : null;
  return typeof ssrWidth === "number" ? ssrWidth : void 0;
}
...
function useStorage2(key, defaults, storage, options = {}) {
  ...
  if (window2 && listenToStorageChanges) {
    tryOnMounted(() => {
      if (storage instanceof Storage)
        useEventListener2(window2, "storage", update, { passive: true });
      else
...
function useElementBounding2(target, options = {}) {
  ...
  watch(() => unrefElement2(target), (ele) => !ele && update());
